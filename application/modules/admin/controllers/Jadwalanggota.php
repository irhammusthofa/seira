...
class Jadwalanggota extends Admin_Controller
{
	...
	public function index()
	{
		...
		$this->content 	= "anggotajadwal/index";
	...
	{
		$this->title 	= "Jadwal Anggota";
		$this->content 	= "anggotajadwal/add";
	...
	public function detekpertemuan($paket,$tgl){
		$paket_anggota = $this->m_paket->by_id($paket)->row();
		$param['paket'] = $paket;
	...
	{
		$this->title 	= "Jadwal Anggota";
		$this->content 	= "anggotajadwal/edit";
		$this->assets 	= array('assets_form');
	...
	public function simpan($id="")
	{
		...
		if (empty($id)){
			...
			if ($ketersediaan['status'] == false && ($data['id_jadwal_instruktur'] != $anggota_jadwal->id_jadwal_instruktur)){
				fs_create_alert(['type'=>'warning','message'=>$ketersediaan['message']]);
				redirect('admin/jadwal/anggota/edit/'.base64_encode($id));
			}else{
	...
	public function ajax_list()
	{
		...
		foreach ($list as $item) {
			...
			$edit = '<li>' . anchor("admin/jadwal/anggota/edit/".base64_encode($item->aj_id),"<i class=\"fa fa-edit\"></i>Edit") . '</li>';
			if (strtotime(date('Y-m-d')) > strtotime(date('Y-m-d',strtotime($item->ji_tgl)))){
				//$edit = '';
			}
			$btngroup = '<div class="input-group">
					...
					<ul class="dropdown-menu">
						'.$edit.'
						<li>' . anchor("admin/jadwal/anggota/hapus/".base64_encode($item->aj_id),"<i class=\"fa fa-trash\"></i>Hapus") . '</li>
					</ul>
                </div>';
			$no++;
