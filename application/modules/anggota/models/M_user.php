...
class M_user extends CI_Model
{
	...
	public function sendemailsignup($data){
		...
		$this->email->from('hadiseptian63@gmail.com', 'Registrasi Seira');
		$this->email->to($data['u_email']);
		$this->email->set_mailtype("html");

		$this->email->subject('Signup');
		$this->email->message('Akun anda berhasil didaftarkan, berikut ini adalah Kode Registrasi anda : <b>'.$data['u_name'].'</b>. Segera aktivasi akun anda melalui Admin, dengan memberikan kode registrasi anda.');
	...
	public function by_id($id){
		return $this->db->from('user u')
			->join('anggota a','a.id_user=u.u_id','inner')
			->where('u.u_id',$id)->get();
	}
