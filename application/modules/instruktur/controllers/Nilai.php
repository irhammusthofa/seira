...
class Nilai extends Instruktur_Controller
{
	...
	public function simpan($id_kategori,$id_paket){
		$data['id_kategori'] = base64_decode($id_kategori);
		$data['id_paket'] = base64_decode($id_paket);
            ...
            for($i=0;$i<=9;$i++){
                if ($i==0){
                    ...
                    if ($nilai['status'] == false){
                    	...
                    	$ceknilai = $this->db
                    		->where('id_jadwal_anggota',$nilai['aj_id'])
                    		->where('id_kriteria',$item->kr_id)
                    		->get('nilai')->row();
                    	if (empty($ceknilai)){
                    		$this->db->insert('nilai',
		                    	[
		                    		'id_jadwal_anggota' => $nilai['aj_id'],
		                    		'nilai' 			=> $this->input->post($item->kr_id.$i,TRUE),
		                    		'id_kriteria' 		=> $item->kr_id,
		                    	]
		                    );
                    	}else{
                    		$this->db
	                    		->where('n_id',$ceknilai->n_id)
	                    		->update('nilai',
		                    	[
		                    		'id_jadwal_anggota' => $nilai['aj_id'],
		                    		'nilai' 			=> $this->input->post($item->kr_id.$i,TRUE),
        ...
        for($i=0;$i<=9;$i++){
            if ($i==0){
                ...
                if ($nilai['status'] == false){
                	...
                	$this->db->where('aj_id',$nilai['aj_id'])->update('anggota_jadwal',
	                    	[
	                    		'aj_absen' 			=> $this->input->post('abs'.$i,TRUE),
	                    	]
	                    );
                }
        ...
        redirect('instruktur/nilai/edit/'.base64_encode($data['id_paket']));
	...
	public function ajax_list()
	{
		...
		foreach ($list as $item) {
			...
			$btngroup = anchor('instruktur/nilai/edit/'.base64_encode($item->p_id),'<i class="fa fa-edit"></i> Edit',array('class'=>'btn btn-xs btn-success'));
